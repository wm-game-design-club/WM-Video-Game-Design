...
EntityDoor = ig.Entity.extend({
	...
	// The level to send the player to. "LevelPlanetA" is an example of the proper format
	targetLevel: "",

	//Name of level to load music for. Example format: planetA
	musicLevel: "",
	...
	check: function(other) {
		...
		if (other instanceof EntityPlayer) {
			if (ig.input.state('action') || this.force) {
				var level = ig.global[this.targetLevel];
				if (level) {
					ig.game.prepareMusic(this.musicLevel);
