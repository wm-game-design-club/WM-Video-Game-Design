...
EntityPlayer = ig.Entity.extend({
	...
	// Boolean that toggles when the player is crouching
	crouching: false,
	...
	update: function() {
		...
		if(ig.input.pressed('crouch')) {
			...
			if(this.currentAnim = this.anims.walk || this.anims.idle) {
				...
				// If player just crouched, move pos so that player doesn't "fall"
				if(this.crouching) this.pos.y = feetPosY - this.size.y;;
