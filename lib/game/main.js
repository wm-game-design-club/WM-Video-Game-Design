...
MyGame = ig.Game.extend({
	...
	loadLevel: function( data ) {
		...
		//will change so that player gets copy of cmap that can then be accessed by enemies. That way its only copied once.
		for(var i=0;i<this.entities.length;i++) {
			if(this.entities[i] instanceof EntityEnemyPursuer) this.entities[i].collisionMap=this.collisionMap;
		}
	},
