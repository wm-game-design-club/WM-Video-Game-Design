...
ig.Game.inject({
	loadLevel: function( data ) {
		...
		this.parent(data);
		if(EntityEnemyPursuer.navMaps.length!==0) {
		alert(0);
		var x=0;
		var y=0;
		for(var i=0;i<EntityEnemyPursuer.navMaps[0].tiles.length;i++) {
			...
			this.backgroundMaps[this.backgroundMaps.length-1].data[y][x]=EntityEnemyPursuer.navMaps[0].tiles[i].type;
		}
		}
		this.backgroundMaps[this.backgroundMaps.length-2].enabled=false;
		this.backgroundMaps[this.backgroundMaps.length-1].enabled=false;
