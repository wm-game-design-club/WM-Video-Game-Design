		...
		if(ig.Entity._debugSight && this instanceof EntityEnemyPursuer) {
			...
			if(delta.error<=1){//if slope makes line closer to x axis, move along x and change y along the way
				...
				for(var i=sightTile.x;step.x*i<=step.x*playerTile.x;i+=step.x) {
					...
					if(i<0||current<0||this.collisionMap.data[current][i]==1) break look;
					this.debugMessages[0]=("oTile: "+i+","+current+" "+this.collisionMap.data[current][i]);
					if(i!=sightTile.x) {
						if(this.collisionMap.data[current][i-step.x]&&this.collisionMap.data[current-step.y][i]) break look;
					}
				...
				for(var i=sightTile.y;step.y*i<=step.y*playerTile.y;i+=step.y) {
					...
					if(i<0||current<0||this.collisionMap.data[i][current]==1) break look;
					this.debugMessages[0]=("oTile: "+current+","+i+" "+this.collisionMap.data[i][current]);
					if(i!=sightTile.y) {
						if(this.collisionMap.data[i-step.y][current]&&this.collisionMap.data[i][current-step.x]) break look;
					}
					error=error+delta.error;
					if(error>=0.0) {
						current+=step.x;
