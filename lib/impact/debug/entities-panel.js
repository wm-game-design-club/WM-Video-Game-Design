		...
		if(ig.Entity._debugSight && this instanceof EntityEnemyPursuer) {
			...
			if(delta.error<=1){//if slope makes line closer to x axis, move along x and change y along the way
				...
				for(var i=sightTile.x;step.x*i<=step.x*playerTile.x;i+=step.x) {
					...
					if(i<0||current<0||current>=EntityEnemyPursuer.collisionMap.data.length||i >= EntityEnemyPursuer.collisionMap.data[current].length||
							EntityEnemyPursuer.collisionMap.data[current][i]==1) break look;
				...
				for(var i=sightTile.y;step.y*i<=step.y*playerTile.y;i+=step.y) {
					...
					if(i<0||current<0||i>=EntityEnemyPursuer.collisionMap.data.length||current >= EntityEnemyPursuer.collisionMap.data[i].length||
							EntityEnemyPursuer.collisionMap.data[i][current]==1) break look;
