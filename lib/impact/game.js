...
ig.Game = ig.Class.extend({
	...
	loadLevel: function( data ) {
		...
		for( var i = 0; i < data.layer.length; i++ ) {
			...
			if( ld.name == 'collision' ) {
				...
				EntityEnemyPursuer.collisionMap = this.collisionMap; //set the static collision map used in enemyPursuer
			}
			else {
		...
		var pursuers= this.getEntitiesByType(EntityEnemyPursuer);
		for(var i=0;i<pursuers.length;i++) pursuers[i].navI=EntityEnemyPursuer.prototype.pushNavMap(pursuers[i].size,pursuers[i].maxVel,pursuers[i].jumpHeight,
																									pursuers[i].gravityFactor, pursuers[i].moveAccel); //set navMaps for pursuers
	...
	function runAfterSeconds(seconds, f) {
		if (seconds > 0.0) {
			coroutines.push({'time': new ig.Timer(seconds), 'f': f});
		} else {
			console.log("Tried to run function after" + seconds + "s");
		}
	}
